...
#  source: https://github.com/ybisk/charNMT-noise
...
if config["ftype"] == 'npy':
    N = np.load(config["file"], allow_pickle=True)
    F = N.tolist()
else:
...
distr = str(config["distribution"][0]).split('.')[0]
if config["ftype"] == "npy":
    sent_list = list()
    for line in F:
        for sentence in iterate_through(line):
            sent_list.append(sentence)
    sent_pd = pd.DataFrame(sent_list)
    sent_np_array = np.array(sent_pd[0])

    np.save(config["file"].split("." + config["ftype"])[0] + "_" + "_".join(config["scrambling"]) + "_" + distr,
            sent_np_array)  # + config["ftype"]
else:
    out = open("%s_%s_%s.%s" % (config["file"].split("." + config["ftype"])[0],
                                    "_".join(config["scrambling"]), distr.split('.')[0],
                                    config["ftype"]), 'w')
    print('out', out)
    if config["ftype"] == "sgm":
        for line in F:
